...
def solve():
    ...
    for i in range(0, n):
        ...
        # print(i, a, b)
        # print()
        # if p[a] > q[b] or (p[a] == q[b] and q[i-a] > p[i-b]):
        #     res = pow(2, p[a]) + pow(2, q[i-a])
        # else:
        #     res = pow(2, p[i-b]) + pow(2, q[b])
        if p[a] > q[b] or (p[a] == q[b] and q[i-a] > p[i-b]):
            res = pow[p[a]] + pow[q[i-a]]
        else:
            res = pow[p[i-b]] + pow[q[b]]
        r += [res % 998244353]
    print(*r)
