...
############################################################
# def solve(): # OLD, TOO LONG
#     n, k =in_ints()
#     a = in_int_list()
#
#     presum = [0]
#     for i in range(0, n):
#         presum += [presum[-1] + int(a[i] <= k)]
#     presum = presum[1:]
#
#     # print()
#     # print(a)
...
#             # print(l, r, presum[l-1], presum[r-1] - presum[l-1], presum[-1] - presum[r-1])
#             a = presum[l-1] >= (float(l) / 2.)
#             b = presum[r-1] - presum[l-1] >= (float(r - l) / 2.)
#             c = presum[-1] - presum[r-1] >= (float(n - r) / 2.)
...
def solve():
    ...
    presum = [0]
    for i in range(0, n):
        presum += [presum[-1] + (1 if a[i] <= k else -1)]
    # presum = presum[1:]
    ...
    min_all = presum[1]
    is_left = presum[1] >= 0
    min_left = presum[1] if is_left else 10**10

    for r in range(2, n):
        if is_left:
            if min_left <= presum[r] or presum[r] <= presum[-1]:
                print('YES')
                return
        if min_all <= presum[r] and presum[r] <= presum[-1]:
            print('YES')
            return

        # Update overall and left mins
        min_all = min(presum[r], min_all)
        if presum[r] >= 0:
            if presum[r] < min_left or not is_left:
                min_left = presum[r]
            is_left = True
